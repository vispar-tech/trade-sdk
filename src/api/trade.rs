...
use crate::{
    error::Result,
    traits::{HttpClient, TradeApi},
    types::{
        AllCategories, ApiResponse, CancelOrderFilter, GetOrderHistoryParams, OrderFilter,
        PlaceOrderParams,
    },
};
...
#[async_trait]
impl<T: HttpClient + Send + Sync> TradeApi for T {
    ...
    async fn cancel_order(
        ...
        let mut api_params = HashMap::new();
        api_params.insert("category".to_string(), category.to_string());
        api_params.insert("symbol".to_string(), symbol.to_string());

        if let Some(order_id) = order_id {
            api_params.insert("orderId".to_string(), order_id.to_string());
        }
        if let Some(order_link_id) = order_link_id {
            api_params.insert("orderLinkId".to_string(), order_link_id.to_string());
        }
        if let Some(order_filter) = order_filter {
            api_params.insert("orderFilter".to_string(), order_filter.to_string());
        }
    ...
    async fn get_open_and_closed_orders(
        ...
        if let Some(base_coin) = base_coin {
            api_params.insert("baseCoin".to_string(), base_coin.to_string());
        }
        if let Some(settle_coin) = settle_coin {
            api_params.insert("settleCoin".to_string(), settle_coin.to_string());
        }
        if let Some(order_id) = order_id {
            api_params.insert("orderId".to_string(), order_id.to_string());
        }
        ...
        if let Some(open_only) = open_only {
            api_params.insert("openOnly".to_string(), open_only.to_string());
        }
        if let Some(order_filter) = order_filter {
            api_params.insert("orderFilter".to_string(), order_filter.to_string());
        }
        if let Some(limit) = limit {
            api_params.insert("limit".to_string(), limit.to_string());
        }
        if let Some(cursor) = cursor {
            api_params.insert("cursor".to_string(), cursor.to_string());
        }
    ...
    async fn cancel_all_orders(
        ...
        if let Some(stop_order_type) = stop_order_type {
            api_params.insert("stopOrderType".to_string(), stop_order_type.to_string());
        }
    ...
    async fn batch_place_order(
        ...
        params.insert("request".to_string(), serde_json::to_string(&request_data)?);
    ...
    async fn batch_cancel_order(
        &self,
        category: AllCategories,
        orders: &[crate::types::CancelOrderParams],
    ) -> Result<ApiResponse<serde_json::Value>> {
